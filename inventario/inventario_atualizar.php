...
if(isset($_POST['consultar'])){
  ...
  $sql = "SELECT * FROM inventario WHERE nome like :nome";
...
// atribuindo valores do bd para preencher no input
  ...
  $tipoconsulta = $usuario["tipo"];
  $quantidadeconsulta = $usuario["quantidade"];
  $valorconsulta = $usuario["valor"];
}
  ...
  if(isset($_POST['deletar'])){
    $nomeguardado = $_POST['nomeguardado'];
    $nomeconsulta = $_POST["nome"];
    $tipoconsulta = $_POST["tipo"];
    $quantidadeconsulta = $_POST["quantidade"];
    $valorconsulta = $_POST["valor"];

    $sql = "UPDATE inventario SET nome = '$nomeconsulta', tipo = '$tipoconsulta', quantidade = '$quantidadeconsulta', valor = '$valorconsulta' WHERE nome = '$nomeguardado'";
    $stmt = $pdo->prepare($sql);
    $stmt->execute();
    $usuario = $stmt->fetch();
  }
    ...
    <div class="d-flex flex-wrap mt-3 flex-column">
      ...
      <form action="" method="post">
          <label for="consultar">Nome:
            <input value = "<?php echo $nomeconsulta ?>" size="25px" type="search" id = "consultar" name = "consultar" placeholder="Fermentador" style="text-align: center;" autofocus>
      ...
      <form method="post" action="" class="caixa">
        ...
        <div class="infos">
          <label for="nome">Nome:
            <input value = "<?php echo $nomeconsulta ?>" type="text" id="nome" name="nome" >
          </label>
        </div>
  
        <div class="infos">
          <label for="tipo">Tipo:
            <input value = "<?php echo $tipoconsulta ?>" type="text" id="tipo" name="tipo" >
          </label>
        </div>
  
        <div class="infos">
          <label for="quantidade">Quantidade:
            <input value = "<?php echo $quantidadeconsulta ?>" type="text" id="quantidade" name="quantidade" >
          </label>
        </div>
  
        <div class="infos">
          <label for="valor">Valor:
            <input value = "<?php echo $valorconsulta ?>" type="text" id="valor" name="valor" >
          </label>
        </div>

        <input type="text" value = "<?php echo $nomeconsulta ?>" name="nomeguardado" id="nomeguardado" hidden>

        <div class="container-fluid mt-3">
          <input class="btn btn-lg btn-primary" style="background-color: green; border: green;" type="submit" name="deletar" id="deletar" value="Salvar">
        </div>
