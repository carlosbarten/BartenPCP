...
// atribuindo valores do bd para preencher no input
  ...
  if(isset($_POST['deletar'])){
    $nome = $_POST['nomeguardado'];
    $sql = "DELETE FROM inventario WHERE nome = :nome";
    $stmt = $pdo->prepare($sql);
    $stmt->bindParam(':nome', $nome);
    $stmt->execute();
    $usuario = $stmt->fetch();
...
<html lang="pt-br">
  <head>
    ...
    <title>Inventário - Deletar</title>
  ...
  <div class="d-flex flex-wrap mt-3 flex-column">
      ...
      <form method="post" action="" class="caixa">
        ...
        <div class="infos">
          <label for="nome">Nome:
            <input value = "<?php echo $nomeconsulta ?>" type="text" id="nome" name="nome" disabled>
          </label>
        </div>
  
        <div class="infos">
          <label for="tipo">Tipo:
            <input value = "<?php echo $tipoconsulta ?>" type="text" id="tipo" name="tipo" disabled>
          </label>
        </div>
  
        <div class="infos">
          <label for="quantidade">Quantidade:
            <input value = "<?php echo $quantidadeconsulta ?>" type="text" id="quantidade" name="quantidade" disabled>
          </label>
        </div>
  
        <div class="infos">
          <label for="valor">Valor:
            <input value = "<?php echo $valorconsulta ?>" type="text" id="valor" name="valor" disabled>
          </label>
        </div>
        ...
        <div class="container-fluid mt-3">
          <input class="btn btn-lg btn-danger" type="submit" name="deletar" id="deletar" value="Apagar">
        </div>
