...
if(isset($_POST["botao_apagar"])){
  ...
  $sql = "DELETE FROM tipo_cerveja WHERE nome = '$nome_atualizar'";
  $stmt = $pdo->prepare($sql);
  $stmt->execute();
  if($stmt->rowCount() > 0){
        ...
        <form class="caixa" action="" method="post">
          ...
          <div class="infos">
            <label for="Volume">Volume:
              <input value = "<?php echo $volume ?>" type="number" id="Volume" name="Volume" disabled>
            </label>
          </div>

          <div class="infos">
            <label for="ABV">ABV:
              <input value = "<?php echo $abv ?>" type="number" id="ABV" name="ABV" disabled>
            </label>
          </div>

          <div class="infos">
            <label for="Amargor">Amargor:
              <input value = "<?php echo $Amargor ?>" type="number" id="Amargor" name="Amargor" disabled>
            </label>
          </div>

          <div class="infos">
            <label for="cor">Cor:
              <input value = "<?php echo $cor ?>" type="number" id="cor" name="cor" disabled>
            </label>
          </div>

          <div class="infos">
            <label for="mostura">Mostura:
              <textarea name="mostura" id="mostura" cols="23" rows="2" disabled><?php echo $mostura ?></textarea>
            </label>
          </div>

          <div class="infos">
            <label for="lavagem">Lavagem:
              <textarea name="lavagem" id="lavagem" cols="23" rows="2" disabled><?php echo $lavagem ?></textarea>
            </label>
          </div>

          <div class="infos">
            <label for="fervura">Fervura:
              <textarea name="fervura" id="fervura" cols="23" rows="2" disabled><?php echo $fervura ?></textarea>
            </label>
          </div>

          <div class="infos">
            <label for="fermentacao">Fermentação:
              <textarea name="fermentacao" id="fermentacao" cols="23" rows="2" disabled><?php echo $fermentacao ?></textarea>
            </label>
          </div>

          <div class="infos">
            <label for="og_rec">OG:
              <input value = "<?php echo $og_rec ?>" type="number" id="og_rec" name="og_rec" disabled>
            </label>
          </div>
          
          <div class="infos">
            <label for="fg_rec">FG:
              <input value = "<?php echo $fg_rec ?>" type="number" id="fg_rec" name="fg_rec" disabled>
            </label>
          </div>

          <div class="infos">
            <label for="tempo_ferm">Dias Fermentação:
              <input value = "<?php echo $temp_ferm ?>" type="number" id="temp_ferm" name="temp_ferm" disabled>
            </label>
          </div>

          <div class="infos">
            <label for="tempo_mat">Dias Maturação:
              <input value = "<?php echo $tempo_mat ?>" type="number" id="tempo_mat" name="tempo_mat" disabled>
            </label>
          </div>
            
          <div class="container-fluid mt-3 mb-4">
            <input class="btn btn-lg btn-danger" id ="botao_apagar" name="botao_apagar" type="submit" value="Apagar">
          </div>
